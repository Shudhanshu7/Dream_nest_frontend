...
const ListingDetails = () => {
  ...
  const getListingDetails = async () => {
    try {
      const response = await fetch(
        `https://rental-app-2.onrender.com/properties/${listingId}`,
        {
          method: "GET",
        }
      );
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await fetch("https://rental-app-2.onrender.com/bookings/create", {
        method: "POST",
        headers: {
      ...
      <div className="listing-details">
        ...
        <div className="photos">
          {listing.listingPhotoPaths?.map((item) => (
            <img
              src={`https://rental-app-2.onrender.com/${item.replace("public", "")}`}
              alt="listing photo"
            />
          ))}
        </div>
        ...
        <h2>
          {listing.type} in {listing.province}, {listing.city},{" "}
          {listing.country}
        </h2>
        <br></br>
        <h2>
        ...
        <div className="profile">
          <img
            src={`https://rental-app-2.onrender.com/${listing.creator.profileImagePath.replace(
              "public",
              ""
            )}`}
